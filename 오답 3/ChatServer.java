...
public class ChatServer {
	...
	//재정의, 채팅방 이름 설정
	public void start(String chatName) throws IOException {
		serverSocket = new ServerSocket(50001);	
		System.out.println( "채팅방 생성");
		try {
			OutputStream os = new FileOutputStream("C:/Temp/"+chatName+".db");//채팅방 이름 생성하고 파일 저장
		} catch (IOException e) {
			e.printStackTrace();
		}
		Thread thread = new Thread(() -> {
	...
	//해당 포트에사람이 있는지 확인
	public boolean userCheck(SocketClient socketClient) {
		if(chatRoom.size()!=0)
			return true;
		return false;
	}
